...
def get_coordinates_from_address(address_text):
    ...
    try:
        ...
        headers = {"User-Agent": "vietnam_post_routing_app_2026_v4"}
    ...
    if activated:
        ...
        if raw_stops:
            try:
                ...
                if len(all_coordinates) >= 2:
                    ...
                    for i in range(len(all_coordinates) - 1):
                        ...
                        try:
                            ...
                            # Nếu OSRM lỗi, vẽ đường thẳng mô phỏng tạm thời
                ...
                st.error(f"Lỗi phân tích hành trình: {e}")
        else:
            with col_left:
                folium_static(m, width=700, height=450)
    else:
        # Mặc định cắm một mốc định vị bưu cục xuất phát trước khi bấm nút tối ưu hóa
        folium.Marker([start_lat, start_lon], tooltip=selected_start_hub, icon=folium.Icon(color='orange', icon='home')).add_to(m)
        with col_left:
            folium_static(m, width=700, height=450)
        with col_right:
            st.info("Nhấn nút 'TỐI ƯU HÓA LỘ TRÌNH THỰC ĐỊA' ở thanh menu bên trái để tính toán và hiển thị các tuyến đường đi!")
...
with tab_order:
    ...
    with st.expander("🛠️ BẢNG ĐIỀU KHIỂN CẬP NHẬT TRẠNG THÁI NHANH", expanded=False):
